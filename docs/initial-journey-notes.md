# Initial Setup Journey Notes

These are my original notes from building this cluster — not a guide, more like a time capsule. Things have moved on. The README covers the current state.
...
this doesn't sit well with me either so stay tuned for an actual solution that's...close to fully automated. Maybe.
...
- Whether you're imaging an ISO onto a bootable USB drive, SD card, DVD, whatever - you will need the appropriate image.
    ...
    - an example image: [`factory.talos.dev/installer/f8a903f101ce10f686476024898734bb6b36353cc4d41f348514db9004ec0a9d:v1.7.5`](https://factory.talos.dev/image/f8a903f101ce10f686476024898734bb6b36353cc4d41f348514db9004ec0a9d/v1.7.5/metal-arm64.raw.xz)
        ...
        - while creating these images yourself, if you WANT persistent storage just make sure to include the `iscsi-tools` and `util-linux-tools` extensions.

## Initial Setup (original flow)
...
1. OPTIONAL: for longhorn, update your generated worker config to include the required mounts (I recommend `cp worker.yaml worker-longhorn.yaml` for these kinds of changes)
    ```yaml
    machine:
        kubelet:
            extraMounts:
              # separate reserved mount for longhorn itself
              # this will use the machine's local disk for longhorn PVs/PVCs
...
```sh
talosctl --nodes <comma-delimited list of controlplane node IPs> \
    apply-config -f controlplane.yaml --insecure


talosctl --nodes <comma-delimited list of WORKER node IPs> \
...
- SLOT THE SD CARD INTO YOUR RPi
    ...
    - after this, poweroff/unplug the RPi, take out your SD card
...
## Proxmox Specific Things (deprecated)

I ran a Proxmox box with a leftover Nvidia GPU, letting me have a VM farm to play with. With that, I was also able to generate TalosOS images to test/play within the cluster _very_ quickly.
